...
```
#!/bin/sh
cd <PUT HERE PATH TO FOCUS>
python <PUT HERE PATH TO FOCUS>/focus.py $1 $2 $3 $4
```
...
motif length from 12 bp to 28 bp. 
...
```
cd perl
perl perl generate_report.pl <directory file> <working directory> <MEME motifs directory> <output file name>
```
...
4.  High number of false peaks complicates motif search. Even if all true 
    ...
    more than 100 input sequences is not recommended, unless you work
    with global regulator that is known to have dozens of site throughout
    the genome.
    

## Additional pipeline steps (optional)

Motif search: you want re-run motif search for one sample with different settings (without
...
Contact authors if you need some sequence data for testing 
